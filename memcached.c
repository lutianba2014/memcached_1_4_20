...
static void complete_nread_ascii(conn *c) {
    ...
    if (strncmp(ITEM_data(it) + it->nbytes - 2, "\r\n", 2) != 0) {
        ...
        fprintf(stderr, "complete_nread_ascii:ITEM_data(it) is %s", ITEM_data(it));
...
enum store_item_type do_store_item(item *it, int comm, conn *c, const uint32_t hv) {
    ...
    if (old_it != NULL && comm == NREAD_ADD) {
        fprintf(stderr, "add is exist but move to the head of lru lst.\n");
        /* add only adds a nonexistent item, but promote to head of LRU */
        do_item_update(old_it);
    } else if (!old_it && (comm == NREAD_REPLACE
        || comm == NREAD_APPEND || comm == NREAD_PREPEND))
    {
        fprintf(stderr, "comm is %d, old_it dont find.\n", comm);
        ...
        if (comm == NREAD_APPEND || comm == NREAD_PREPEND) {
            ...
            if (stored == NOT_STORED) {
                ...
                /*考虑一下优化,可以先判断old_it的内存是否足够，如果足够就直接使用，否则在分配新的内存*/
...
static enum try_read_result try_read_network(conn *c) {
    ...
    while (1) {
        ...
        if (res > 0) {
            ...
            c->rbytes += res;
            if (res == avail) {//表示没有内存可供读取，在分配内存
                continue;
            } else {//数据读完 返回
                fprintf(stderr,"c-rbuf=%s\n", c->rbuf);
                break;
            }
        }
        if (res == 0) {
