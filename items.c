...
/*@null@*/
/*
 *计算需要的内存大小 sizeof(item)+(nkey+1)+nsuffix+nbytes+cas
 * */
item *do_item_alloc(char *key, const size_t nkey, const int flags,
    ...
    for (; tries > 0 && search != NULL; tries--, search=search->prev) {
        if (search->nbytes == 0 && search->nkey == 0 && search->it_flags == ITEM_LINKED) {
    ...
    it->nsuffix = nsuffix;
    fprintf(stderr,"it->it_flags=%d.\n",it->it_flags);
    fprintf(stderr,"it->nkey=%d.\n",it->nkey);
    fprintf(stderr,"it->nbytes=%d.\n",it->nbytes);
    fprintf(stderr,"key=%s.\n",key);
    fprintf(stderr,"it->exptime=%d.\n",it->exptime);
    fprintf(stderr,"suffix=%s,nsuffix=%d.\n",suffix,nsuffix);
    return it;
}
...
item *do_item_get(const char *key, const size_t nkey, const uint32_t hv) {
    ...
    if (it != NULL) {
        ...
        } else if (it->exptime != 0 && it->exptime <= current_time) {/*是否设置的时间超期*/
            do_item_unlink(it, hv);
            do_item_remove(it);
            it = NULL;
            if (was_found) {
...
/*LRU,根据每个slabs ID下面的head tail链表，去掉超期的内存块*/
